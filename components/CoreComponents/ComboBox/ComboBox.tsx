...
const ComboBox: React.FC<ComboBoxProp> = (props) => {
    const { options, placeHolder, type, id, size,
        ...
        notfound, readonly, customCSS } = props
    const {
      inputRef,
      dropdownRef,
      openListHandler,
    ...
    return (
      <div
        className={`relative w-[300px]  m-2 ${customCSS || ''}  ${h_container}`}
        ref={dropdownRef}
        ...
        <div className="w-full h-full flex relative items-center justify-between">
          <input
            ...
            ref={inputRef}
            type={type}
            onChange={(event) => searchOptionsHandler(event)}
        ...
        {isOpen && (
          <div
            className={`absolute z-10 ${position_modal} left-0 right-0 bg-white border rounded shadow`}>
            {searchTerm?.length === 0 && <div className="p-2">{notfound}</div>}
